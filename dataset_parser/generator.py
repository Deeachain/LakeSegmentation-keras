...
def get_result_map(b_size, y_img):
    ...
    result_map = np.zeros((b_size, 512, 512, 2))

    # For np.where calculation.
    lake = (y_img == 255)
    _background_ = (y_img == 0)
    result_map[:, :, :, 0] = np.where(_background_, 1, 0)
    result_map[:, :, :, 1] = np.where(lake, 1, 0)
...
def data_generator(d_path, b_size, mode):
    ...
    while True:
        ...
        for i in range(d_size):
            idx = shuffled_idx[i]
            x.append(x_imgs[idx].reshape((512, 512, 3)))
            y.append(y_imgs[idx].reshape((512, 512, 1)))
